...
class slice_view : public std::ranges::view_interface<slice_view<V>> {
    ...
    class sentinel {
      ...
      public:
        ...
        friend constexpr bool operator==(const std::counted_iterator<std::ranges::iterator_t<V>>& x,
                                         const sentinel&                                          y) {
  ...
  public:
     ...
     */
    constexpr explicit slice_view(V                                  base,
    ...
    constexpr auto begin()
        requires(!(detail::simple_view<V> && std::ranges::random_access_range<const V> &&
                   std::ranges::sized_range<const V>))
    {
        if constexpr (std::ranges::sized_range<V>) {
            ...
            return counted_iterator(std::ranges::next(std::ranges::begin(base_), from_, std::ranges::end(base_)),
                                    to_ - from_);
        }
    }
...
slice_view(R&&, std::ranges::range_difference_t<R>, std::ranges::range_difference_t<R>)
    -> slice_view<std::ranges::views::all_t<R>>;
...
struct slice_closure : std::ranges::range_adaptor_closure<slice_closure<D>> {
    ...
    constexpr auto operator()(R&& r) const {
        ...
        else if constexpr ((is_iota_view<T> || is_subrange_v<T>) && std::ranges::random_access_range<T> &&
